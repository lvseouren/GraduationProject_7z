...
{
	//std::cout<<"fuck fuck!!!!!!!!!!!!!"<<endl;    函数的最初入口
  #if defined(_WIN32) && !defined(UNDER_CE)
  SetFileApisToOEM();
  #endif
  
  UStringVector commandStrings;   //怀疑是装命令参数的
  #ifdef _WIN32
  ...
  if (commandStrings.Size() == 1)	//没有正确给参数情况
  {
  ...
  parser.Parse1(commandStrings, options);   //看第二个参数，就是要这个软件干嘛
  ...
  if (options.EnableHeaders){
	  //std::cout<<"in options.EnableHeaders"<<std::endl;
    ShowCopyrightAndHelp(stdStream, false);    //参数一个代表输出流，一个代表是否要输出用法
  }

  parser.Parse2(options);

  CCodecs *codecs = new CCodecs;  //这东西就是用来处理解压缩的
  CMyComPtr<
    #ifdef EXTERNAL_CODECS
    ICompressCodecsInfo
    #else
    IUnknown
    #endif
    > compressCodecsInfo = codecs;     //给自定义类型进行了海量的符号重载

  HRESULT result = codecs->Load();   //这个load真有点不明白，load了11个东西···我去，但是不同文件都一样
  if (result != S_OK)
    throw CSystemException(result);

  bool isExtractGroupCommand = options.Command.IsFromExtractGroup();   //看命令的类型
  ...
  {
	   std::cout<<"run in here!!!!!!!!!!!!!"<<std::endl;
    if (isExtractGroupCommand)
    {	
	  std::cout<<"run in here too!!!!!!!!!!!!!"<<std::endl;
      CExtractCallbackConsole *ecs = new CExtractCallbackConsole;  //用来配置密码之类的信息
      CMyComPtr<IFolderArchiveExtractCallback> extractCallback = ecs;   //回调函数···不知道想作甚

      ecs->OutStream = &stdStream;    //设定输出在屏幕
      ...
      ecs->Init();   //初始化一些数据为0而已

      COpenCallbackConsole openCallback;   //用来配置密码之类的信息
      ...
      CDecompressStat stat;   //几个状态变量
	 // std::cout<<"1231231233!!!!!!!!!!!!!"<<endl;
	  //下面这个函数是主要的提取工作开始
