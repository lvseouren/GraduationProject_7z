...
struct CWzAesExtraField
{
  ...
  CWzAesExtraField(): VendorVersion(2), Strength(3), Method(0) {}			//默认帮设置好所以初始值了

  bool NeedCrc() const { return (VendorVersion == 1); }

  bool ParseFromSubBlock(const CExtraSubBlock &sb)			//提取头部相当于
  {
    if (sb.ID != NFileHeader::NExtraID::kWzAES)
      return false;
    if (sb.Data.GetCapacity() < 7)
      return false;
    const Byte *p = (const Byte *)sb.Data;			//怀疑是移动到加密数据当前位置
...
struct CExtraBlock
{
  ...
  {
	//printf("check SubBlocks.Size():%d\n",SubBlocks.Size());		block.size=2   为什么是2就不清楚了
    for (int i = 0; i < SubBlocks.Size(); i++)
      if (aesField.ParseFromSubBlock(SubBlocks[i]))
	  {
		//printf("check aesField.ParseFromSubBlock aesField.ParseFromSubBlock\n");		//会显示~
        return true;
	  }
    return false;
  }
